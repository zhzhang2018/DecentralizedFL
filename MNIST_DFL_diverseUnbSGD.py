...
# There aren't too many changes compared to MNIST_DFL_overlapData.py and others.
...
#                         new_models[i][key] += avg_weight * adj[i,j] * clients[j].model.state_dict()[key]
            ...
            # We have several different ways of achieving consensus, but we mostly use the first one.
            if avg_alg == 'epsilon':
                ...
                # Metropolis update rule is basically weighted average
                ...
                # Direct averaging in a discrete way - might fail.
                for i in range(len(clients)):
                    Ni = np.sum(adj[i,:])
...
import utils
# Parameters that affect the partition sizes
S = 1.0
Mean = 1
V = 0.7
...
# Determine which loss function to use for each client
loss_fs = [F.nll_loss] * K
list_size_counts = [K]

# Based on the argument sys.argv[2], choose the optimizers that are going to be used.
# They're represented by A-E as shorthand notations. Each has their default learning rates below.
...
# If 'F' is used, then the identifier should have the form "F_xy(mns)", where 'x' is the first optimizer, 
# 'y' is the second (both using default learning rates), and (mns) is one of the m/n/s characters that 
# would describe how many clients get the first optimizer and how many gets the second.
if file_type[0] == 'F':
    if file_type[-1] == 'm':
...
# If only one single optimizer is used, then the number following the alphabet would indicate the learning rate.
if file_type[0] == 'A':
    lr_list = [0.001] * K
    if file_type == 'A2':
...
# F stands for Fusion.
# If F is used, then here we assign the optimizers (with counts specified previously)
elif file_type[0] == 'F':
    if file_type == 'F_AC': 
...
#elif - ignore these two. They're seldom used and are probably obsolete.
elif file_type == 'X':
...
script_filiename = 'MNIST_DFL_diverseSGD{}_{}dataset_K{}_C{}_E{}_S{}_M{}_V{}_'.format(file_type,unb_name,K,C,E*EE,S,Mean,V)
...
# Optional: You can also use datasets with only one label across, i.e. skewed dataset partitioning.
# The code block below performs such partitioning.
# If a line of code is commented out, you can ignore it. It's commented out because it doesn't work.
if use_unbalanced_dataset:
    ...
    for i in range(10):
#         # Shuffle each class' data prior to partitioning
#         indices = np.arange(len(datasets_by_label[i].data))
#         np.random.shuffle(indices)
#         datasets_by_label[i].data = datasets_by_label[i].data[np.r_[indices]]
#         datasets_by_label[i].targets = datasets_by_label[i].targets[np.r_[indices]]
...
#     class_client_sample_count = class_client_sample_count.astype(int)
...
#     empty_dataset = copy.deepcopy(dataset1)
#     empty_dataset.data = []
#     empty_dataset.targets = []
...
#         partitioned_full_dataset.append(copy.deepcopy(empty_dataset))
    for i in range(10):
#         dataset_ind_ref = np.insert( np.cumsum(class_client_sample_count[i]), 0, 0 )
        for j in range(K):
            if i == 0:
                partitioned_full_dataset.append( partitioned_full_clientwise_dataset[i][j] )
#                 partitioned_full_dataset.append( copy.deepcopy(partitioned_full_clientwise_dataset[i][j]) )
            else:
                partitioned_full_dataset[j] += partitioned_full_clientwise_dataset[i][j]
#                 partitioned_full_dataset[j] += datasets_by_label[i][dataset_ind_ref[j]:dataset_ind_ref[j+1]]
...
# If some data would be shared, then the code below makes that sharing possible.
partitioned_dataset = []
for i in range(K):
...
script_filiename = script_filiename + file_identifier
for i in range(K):
