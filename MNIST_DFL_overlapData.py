...
# Basic network structure. "forward()" determines how the network processes the input and produces output.
class Net(nn.Module):
    def __init__(self):
...
# Parameters for training (mostly from PyTorch examples; not all are used)
batch_size = 64
...
# Load and pre-process training and testing data (normallization)
...
import copy
# Implement server logic (FedAvg; DFL shouldn't need to use this)
class ServerNet():
    def __init__(self):
...
class ClientNet():
    ...
    # Trains on the assigned training dataset for one epoch (or less).
    # self.E determines how many times do the client pass through the training set.
    # max_n determines how many samples to train on before breaking the loop - effectively implementing EE.
    # The value self.E * EE would be equivalent to the "E" shown in the report.
    def update(self, dry_run=False, max_n=np.inf, weights=None):
        ...
        for e in range(self.E):
            ...
            for batch_idx, (data, target) in enumerate(self.train_loader):
                # Load data and train data by batches
...
def train_DFL(adj, clients, dists, num_rounds=1, dry_run=False, debug=False, CK=10, max_n=np.inf, test_loader=None,
    ...
    # past_params help you pick up the training from where you left off. Assumes that the first two things
    # in past_params are lists, and have the correct length.
    test_loss = []
    test_accuracy = []
    start_round_ind = 0
        ...
        for i in range(K):
            for j in range(K):
                ...
                if j>i: # Avoid double checking
                    ...
                    # dists[i,j,:,:] would store the model distance history between client i and client j.
                    # dists[i,j,0,e] stores the distance prior to aggregation at the e-th epoch.
                    dists[i,j,0,e] = client_wise_dist

        reached_consensus = False
        # Start model aggregation logic. It's the consensus protocol. 
...
import utils
S = 0.3 # Fraction of overlapping datapoints. When all agents have full access, you get S = K-1.
...
script_filiename = 'MNIST_DFL_overlap_dataset_K{}_C{}_E{}_S{}_M{}_V{}_'.format(K,C,E*EE,S,Mean,V)
...
# Turns out that you can concatenate datasets by simply using +, so here's the plan:
# First, we generate random sizes for each client's initial dataset.
# Next, according to S, we calculate the number of samples each dataset has to share to others.
# Finally, for each client, we loop with other clients' datasets and randomly pick some samples (of that number) to share.
# Without using any randomness in this number calculating process, we can do this outside the read loop, and reproduce it.
