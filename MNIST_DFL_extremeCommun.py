...
#                     print("Breaking at: ", batch_idx, batch_idx*self.B, " / ", len(self.train_loader.dataset), ", with current batch size = ", len(data))
# batch_idx grows 1 at a time. The loop will be over when all data is used, and batch_idx*self.B might be larger than that.
                    break
#                 else:
#                     print("We're now at: ", batch_idx, batch_idx*self.B, " / ", len(self.train_loader.dataset), ", with current batch size = ", len(data))
...
def train_DFL(adj, clients, dists, comm_costs, samples_seen, 
...
#                         new_models[i][key] -= clients[i].model.state_dict()[key]
        ...
        if test_loader is not None:
            if test_all_client and (not reached_consensus):
                test_loss.append([])
                saccus = []
...
script_filiename = 'MNIST_DFL_extCom{}_{}dataset_K{}_C{}_E{}_S{}_M{}_V{}_'.format(file_type,unb_name,K,C,E*EE,S,Mean,V)
...
DFL_loss_history, DFL_accuracy_history = train_DFL(Adj, clients_DFL, dists, comm_costs, samples_seen, 
