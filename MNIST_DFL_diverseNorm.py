...
if use_same_base_norm:
    ...
    for i in range(K):
        ...
        norm_records.append( (norm_mean + ((i // (int(K/2))) - 0.5)*2 * 0.1, 
                              norm_stdev+ ((i // (int(K/2))) - 0.5)*2 * 0.01) )
        # Method 2: Use random
#         norm_records.append( (norm_mean + (np.random.random()*2 - 1) * 0.1, 
#                               norm_stdev+ (np.random.random()*2 - 1) * 0.01) )
else:
    # Option 2: Use the mean and stdev from each (base) dataset.
    # The problem here is that you can't easily access the mean and stdev of each client's local dataset,
    # after they have combined with other partitions and become a ConcatDataset. ConcatDatasets don't have
    # attributes like .data or .targets... so it gets awkward. We have to use only the base dataset's values,
    # i.e. use the partition with only one class.
    for i in range(K):
        norm_records.append( (partitioned_full_dataset[i].datasets[0].data.float().mean(), 
                              partitioned_full_dataset[i].datasets[0].data.float().std()) )
