...
last_opt_vals = [0.1, 0.5, 0.8, 0.9, 1.0]
...
for pbs_id in pbs_ids:
    for opt_val in last_opt_vals:
        filename = pbs_name+'_MNIST0401redcom_'+pbs_id+name_the_rest+filename_the_rest+pbs_opts[-1]+'{}.pbs'.format(opt_val)
        with open(filename, 'w') as fp:
            fp.writelines(['#!/bin/bash\n',
                       '#PBS -N red0401_'+pbs_opts[-1]+'{}'.format(opt_val)+pbs_id+'\n',
                       '#PBS -A GT-cabdallah3\n',
                       ...
                       'cd $PBS_O_WORKDIR\n',
                       'module load pytorch\n',
                       'python3 MNIST_DFL_reducedDebug0401.py '+pbs_name+' '+pbs_id+the_rest+' '+pbs_opts[-1]+' {}'.format(opt_val),
                       '\n\n#END OF SCRIPT'
                      ])
