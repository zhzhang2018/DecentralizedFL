...
if use_same_base_norm:
    ...
    for i in range(K):
        ...
        norm_records.append( (norm_mean + ((i // (int(K/2)-1)) - 0.5)*2 * 0.2, 
                              norm_stdev+ ((i // (int(K/2)-1)) - 0.5)*2 * 0.01) )
        # Method 2: Use random
...
#                               norm_stdev+ (np.random.random()*2 - 1) * 0.01) )
    ...
    for i in range(K):
        norm_records.append( (partitioned_dataset[i].data.float().mean(), 
                              partitioned_dataset[i].data.float().std()) )
