            ...
            if len(test_accuracy) > 50:
                ...
                if (( np.mean(test_accuracy[-1]) < np.mean(test_accuracy[-50])-5 ) and 
                   ...
                   ( np.mean(test_accuracy[-1]) < 90 )):
                    break_bc_drop = True
                if (( np.mean(test_accuracy[-1]) < 20 ) and 
                   ( np.mean(test_accuracy[-20]) < 20 ) and 
                   ( np.mean(test_accuracy[-50]) < 20 )):
                    break_bc_low = True
...
num_rounds = 200
EE = 5 # (1/num_rounds) # Portion of data per epoch (<= 1)
max_n = n/K * EE
...
DFL_loss_history, DFL_accuracy_history = train_DFL(AK, clients_DFL, dists, num_rounds=num_rounds, dry_run=False, 
              ...
              num_avg_iter=num_avg_iter, max_num_avg_iter=10, avg_error_thres=0.01,
              past_params=params, save_model_to=script_filename+file_identifier, bother_saving=2, 
