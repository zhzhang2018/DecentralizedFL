# This one allows a common set of training data for each client
import torch
...
def train_DFL(adj, clients, dists, num_rounds=1, dry_run=False, debug=False, CK=10, max_n=np.inf, test_loader=None,
    ...
    for e in range(start_round_ind,num_rounds):
        ...
        np.save("Finished {}th".format(e), [0])
        K = adj.shape[0]
...
E = 5 # Epochs per client update (>= 1)
n = len(dataset1) # Number of training samples in total
num_rounds = 200
EE = 1 # (1/num_rounds) # Portion of data per epoch (<= 1)
max_n = n/K * EE

S = 0.5 # Fraction of datapoints commonly available. Must be within [0,1].
n_s = int(S*n)
n_p = n - n_s
nk = [int(n_p/K)]*K
nk[-1] = n_p - sum(nk[:-1])
...
script_filiename = 'MNIST_DFL_repeated_dataset_K{}_C{}_E{}_S{}_M1_V0_'.format(K,C,E*EE,S)
...
# Following results in: [#1's private dataset, #2's private, ..., #K's private, all clients' shared dataset]
partitioned_dataset = torch.utils.data.random_split(dataset1, nk)
torch.manual_seed(seed) # Return to previous seed

# Create agents and assign datasets
clients_DFL = []
for i in range(K):
    # https://pytorch.org/docs/master/data.html#torch.utils.data.ConcatDataset
    clients_DFL.append(ClientNet(
        partitioned_dataset[i]+partitioned_dataset[-1],
#            torch.utils.data.ConcatDataset(partitioned_dataset[i], partitioned_dataset[-1]), 
        B, E))
...
# # Only save the first and last, because # of iterations is uncertain
DFL_loss_history, DFL_accuracy_history = train_DFL(AK, clients_DFL, dists, num_rounds=num_rounds, dry_run=False, 
              ...
              past_params=params, save_model_to=script_filename+file_identifier, bother_saving=3)
