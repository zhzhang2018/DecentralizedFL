    ...
    for S in S_vals:
        with open(pbs_name+'_'+pbs_id+name_the_rest+'S{}.pbs'.format(S), 'w') as fp:
            fp.writelines(['#!/bin/bash\n',
                       '#PBS -N DFL_CIFARTF_'+pbs_id+name_the_rest+'\n',
                       '#PBS -A GT-cabdallah3\n',
                       '#PBS -l nodes=1:ppn=16\n',
                       '#PBS -l pmem=2gb\n',
                       '#PBS -l walltime=07:49:00\n',
                       '#PBS -j oe\n',
                       '#PBS -o divSGD_CIFARTF_'+pbs_name+pbs_id+name_the_rest+'S{}.out\n'.format(S),
                       '#PBS -m abe\n',
                       '#PBS -M zzhang433@gatech.edu\n\n',
                       'cd $PBS_O_WORKDIR\n',
                       'module load pytorch\n',
                       'python3 CIFARTF_DFL_diverseSGD.py divSGD_CIFARTF_ '+pbs_id+the_rest+' -s {}'.format(S),
                       '\n\n#END OF SCRIPT'
                      ])
