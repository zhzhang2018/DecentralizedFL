...
##                print(target, len(self.train_loader.dataset))
...
if use_unbalanced_dataset:
    ...
    class_share_percentage = 1.0 #0.5 # 0.0 # 0.1
    ...
    print(class_assign_ind, class_dominate_counts)
    # Assign percentages
    ...
    for i in range(10):
        ...
        class_client_sample_count[i] = rand_for_class * len(datasets_by_label[i]) # len(indices)
...
#    print(class_client_sample_count)
    print(type(class_client_sample_count[0][0]), type(class_client_sample_count[0][-1]))
...
##                print(i,j, datasets_by_label[i].targets[np.r_[partitioned_full_clientwise_dataset[i][j].indices]])
#                 partitioned_full_dataset[j] += datasets_by_label[i][dataset_ind_ref[j]:dataset_ind_ref[j+1]]
#            print(dataset_ind_ref)
#            print(datasets_by_label[i])
#       	    print(datasets_by_label[i][0])
#            partitioned_full_dataset[j] += datasets_by_label[i][dataset_ind_ref[j]:dataset_ind_ref[j+1]]
...
script_filiename = script_filiename + file_identifier + '_classp_{}_'.format(class_share_percentage)
for i in range(K):
