    ...
    for S in S_vals:
        with open(pbs_name+'_'+pbs_id+name_the_rest+'S{}.pbs'.format(S), 'w') as fp:
            fp.writelines(['#!/bin/bash\n',
                       ...
                       '#PBS -l nodes=1:ppn=8\n',
                       '#PBS -l pmem=2gb\n',
                       '#PBS -l walltime=05:29:00\n',
                       '#PBS -j oe\n',
                       '#PBS -o divSGDebug_0.0_'+pbs_id+name_the_rest+'S{}.out\n'.format(S),
                       '#PBS -m abe\n',
                       '#PBS -M zzhang433@gatech.edu\n\n',
                       'cd $PBS_O_WORKDIR\n',
                       'module load pytorch\n',
                       'python3 MNIST_DFL_diverseUnbSGDebugger.py divUnbE1Class0 '+pbs_id+the_rest+' -s {}'.format(S),
                       '\n\n#END OF SCRIPT'
                      ])
