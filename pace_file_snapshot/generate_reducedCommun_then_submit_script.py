...
for pbs_id in pbs_ids:
    for opt_val in last_opt_vals:
        filename = pbs_name+'_CIFARredcom_'+pbs_id+name_the_rest+filename_the_rest+pbs_opts[-1]+'{}.pbs'.format(opt_val)
        with open(filename, 'w') as fp:
            fp.writelines(['#!/bin/bash\n',
                       ...
                       '#PBS -j oe\n',
                       '#PBS -o redcommunCIFAR_'+pbs_name+pbs_id+filename_the_rest+pbs_opts[-1]+'{}.out\n'.format(opt_val),
                       '#PBS -m abe\n',
                       '#PBS -M zzhang433@gatech.edu\n\n',
                       'cd $PBS_O_WORKDIR\n',
                       'module load pytorch\n',
                       'python3 CIFAR_DFL_reducedSharing.py '+pbs_name+' '+pbs_id+the_rest+' '+pbs_opts[-1]+' {}'.format(opt_val),
                       '\n\n#END OF SCRIPT'
                      ])
