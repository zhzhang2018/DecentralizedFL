...
class ClientNet():
    def __init__(self, dataset, B=16, E=1, optimizerlr=1, schedgamma=0.7, optim_method=optim.Adadelta, loss_f=nn.CrossEntropyLoss):
        ...
        if loss_f == nn.CrossEntropyLoss:
            self.target_processing = lambda x : x
            print("Picked x=x for loss ", loss_f, "; ", optimizerlr)
        elif loss_f == nn.BCEWithLogitsLoss:
            self.target_processing = lambda x : (F.one_hot(x).to(torch.float))
            print("Picked x=one_hot(x) for loss ", loss_f, "; ", optimizerlr)
#        print(self.loss_f, self.loss_f==nn.CrossEntropyLoss, optimizerlr, self.target_processing)
...
#                print(target, type(target))
                target = self.target_processing(target)
...
#                 test_accuracy.append(np.max(saccu))
            ...
            if len(test_accuracy) > 50:
                break_bc_low = False
                break_bc_drop = False
                if (( np.mean(test_accuracy[-1]) < np.mean(test_accuracy[-50])-0.05 ) and 
                   ( np.mean(test_accuracy[-20]) < np.mean(test_accuracy[-50])-0.03 ) and 
                   ( np.mean(test_accuracy[-5]) < np.mean(test_accuracy[-50])-0.05 ) and 
                   ( np.mean(test_accuracy[-1]) < 0.9 )):
                    break_bc_drop = True
                if (( np.mean(test_accuracy[-1]) < 0.5 ) and 
                   ( np.mean(test_accuracy[-20]) < 0.5 ) and 
                   ( np.mean(test_accuracy[-50]) < 0.7 )):
                    break_bc_low = True
...
if file_type[0] == 'X':
    ...
    print(bxelwl_count, loss_fs, file_type)
if file_type[0] == 'F':
    if file_type[-1] == 'm':
...
script_filiename = 'MNIST_DFL_diverseLoss{}_{}dataset_K{}_C{}_E{}_S{}_M{}_V{}_'.format(file_type,unb_name,K,C,E*EE,S,Mean,V)
