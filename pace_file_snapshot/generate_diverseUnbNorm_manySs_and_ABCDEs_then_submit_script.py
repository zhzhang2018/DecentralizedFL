    ...
    for S in S_vals:
        ...
        with open(filename, 'w') as fp:
            fp.writelines(['#!/bin/bash\n',
                       '#PBS -N DFL_divnorm_2_'+pbs_id+name_the_rest+'\n',
                       '#PBS -A GT-cabdallah3\n',
                       '#PBS -l nodes=1:ppn=8\n',
                       '#PBS -l pmem=2gb\n',
                       '#PBS -l walltime=06:55:00\n',
                       '#PBS -j oe\n',
                       '#PBS -o divnorm_2_'+pbs_name+pbs_id+name_the_rest+'S{}.out\n'.format(S),
                       '#PBS -m abe\n',
                       '#PBS -M zzhang433@gatech.edu\n\n',
                       'cd $PBS_O_WORKDIR\n',
                       'module load pytorch\n',
                       'python3 MNIST_DFL_diverseUnbNorm.py '+pbs_name+' '+pbs_id+the_rest+' -s {}'.format(S),
                       '\n\n#END OF SCRIPT'
                      ])
