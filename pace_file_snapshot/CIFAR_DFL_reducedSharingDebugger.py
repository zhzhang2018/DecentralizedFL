...
class ParameterSharer():
    ...
    def __init__(self, clients, nk, A, R=1.0, share_mode='l', 
        ...
        for key in self.clients[0].model.state_dict().keys():
            ...
            print(key, self.clients[0].model.state_dict()[key].shape)
            print(self.clients[0].model.state_dict()[key][0].shape, (self.clients[0].model.state_dict()[key][0]+self.clients[0].model.state_dict()[key][1]).shape)
            print(self.clients[0].model.state_dict()[key].shape[0], type(self.clients[0].model.state_dict()[key].shape[0]))
        if self.mode == 'l':
            # Share by layers
    ...
    def run_epoch_update(self):
        ...
        dists = np.zeros((K,K,1000)) # Stores pairwise distances
...
def train_DFL(adj, clients, dists, comm_costs, samples_seen, sharer,
    ...
    for e in range(start_round_ind,num_rounds):
        ...
        ccost, kdist, kdistind = sharer.run_epoch_update()
