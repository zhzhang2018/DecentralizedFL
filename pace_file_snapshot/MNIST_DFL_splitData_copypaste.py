...
try:
    with open(script_filiename+file_identifier, 'rb') as fp:
        params = pickle.load(fp)
    print("Successfully loaded pickle")
    for i,cl in enumerate(clients_DFL):
        cl.model.load_state_dict(torch.load(script_filiename+file_identifier+'_client{}'.format(i)))
except:
    params = None
    print("Did not find file that starts with '"+script_filiename+file_identifier+"'. Starting from scratch.")
...
DFL_loss_history, DFL_accuracy_history = train_DFL(AK, clients_DFL, dists, num_rounds=num_rounds, dry_run=True,#False, 
              ...
              past_params=params, save_model_to=script_filiename+file_identifier, bother_saving=10)
