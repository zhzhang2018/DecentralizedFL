...
#                         new_models[i][key] += avg_weight * adj[i,j] * clients[j].model.state_dict()[key]
        ...
        if test_loader is not None:
            ...
            if len(test_loss) > 50:
                break_bc_low = False
                break_bc_drop = False
                if (( np.mean(test_loss[-1]) < np.mean(test_loss[-50])-0.05 ) and 
                   ( np.mean(test_loss[-20]) < np.mean(test_loss[-50])-0.03 ) and 
                   ( np.mean(test_loss[-5]) < np.mean(test_loss[-50])-0.05 ) and ( np.mean(test_loss[-1]) < 0.9 )):
                    break_bc_drop = True
                if (( np.mean(test_loss[-1]) < 0.5 ) and 
                   ( np.mean(test_loss[-20]) < 0.5 ) and 
                   ( np.mean(test_loss[-50]) < 0.7 )):
                    break_bc_low = True
...
#            print(data.data.float().min(), data.data.float().max())
            data, target = data.to(device), target.to(device)
...
num_rounds = 500
EE = 1/100 # (1/num_rounds) # Portion of data per epoch (<= 1)
max_n = n/K * EE

# nk = [int(n/K)]*K
# nk[-1] = n - sum(nk[:-1])
import utils
nk = utils.generate_partition_sizes(K, n, mean=1, variance=0.7, options=None, minsize=B)
...
if use_unbalanced_dataset:
    file_type += 'Unbalanced'

script_filiename = 'MNIST_DFL_A_split{}dataset_K{}_C{}_E{}_'.format(file_type,K,C,E*EE)
#script_filiename = 'MNIST_DFL_split_dataset_K{}_C{}_E{}_'.format(K,C,E*EE)
...
if use_unbalanced_dataset:
    ...
    for i in range(10):
        ...
        #print(datasets_by_label[-1].data.float().min(), datasets_by_label[-1].data.float().max())
    ...
    partitioned_dataset = datasets_by_label
...
#        print(datasets_by_label[i])
        print(datasets_by_label[i].transform)
else:
...
DFL_loss_history, DFL_accuracy_history = train_DFL(AK, clients_DFL, num_rounds=num_rounds, dry_run=False, 
                                                   CK=K, max_n=max_n, 
