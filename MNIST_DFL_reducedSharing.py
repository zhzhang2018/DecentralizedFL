...
class ParameterSharer():
    ...
    # The previous code files have been overhauled, so that some of the functionalities in train_DFL() were 
    # moved into this class' methods.
    # This class mainly determines how the segments of each client's model would be assigned, and averaged.
    # It creates an array of indices to keep track of who shares what with whom, and then performs the
    # aggregation steps based on those assignments.
    ...
    def __init__(self, clients, nk, A, R=1.0, share_mode='l', 
                 avg_alg='epsilon', use_rand=False, crs=False, srn=False,
                 percentage_shared=1, consensus_threshold=0.05, 
        ...
        self.client_rand = crs # Whether different clients have randomly different segments of interest
        self.segwise_SW  = srn # Whether generate a different small-world topology for each segment
        ...
        # Internal aids:
        self.K = len(clients)
        self.numLayer = 4 # Only counts layers with parameters. Note that each layer has weights and biases.
...
#             print("Generating a new share map thing")
        self.seg_dists = None       # Only stores for one epoch for one client, for the sake of speed & memory
        self.seg_dist_params = None
...
#             np.save(self.loadname, self.share_maps)
    ...
    def split_filters(self):
        ...
        # (Each clients shares and receives a different set of weights??)
        ...
        for i in range(self.K):
            ...
            for j,find in enumerate(filter_inds):
                ...
                if Nis[i] <= self.minNumNeighbor:
                    ...
                    R = max(self.minNumNeighbor, int(Nis[i] * self.R))
                # [0] stores polling neighbor index (varies). [1,2] stores filter index in the network (same).
    ...
    def split_channels(self):
        ...
        segA = self.A
        if self.segwise_SW:
            ### WARNING: Might be incompatible with 'cu', because the implementation uses self.A for 'cu' later.
            # The best idea for now is to always set self.A as complete graph.
            # This code right now assumes to, and creates small-world based on such assumption. Later we could
            # try to coordinate and make consensus work for 'cu' while creating SW from self.A instead.
            segA = np.zeros((num_filters, self.K, self.K))
            for s in range(segA.shape[0]):
                AC = np.zeros((self.K, self.K))
                ...
                for i in range(1,self.K):
                    for j in range(i,self.K):
                        if AC[i,j] == 1 and np.random.random() < p:
                            ...
                            i_add = np.random.randint(1,self.K)
                            if AC[i,(j+i_add)%self.K] == 0 and i != (j+i_add)%self.K:
                                segA[s,i,j] = 0
                                segA[s,j,i] = 0
                                segA[s,i,(j+i_add)%self.K] = 1
                                segA[s,(j+i_add)%self.K,i] = 1
        if self.mode == 'c':
            for i in range(self.K):
                if self.client_rand:
                    chosen_filters = np.random.permutation(np.arange(num_filters))[:int(num_filters*self.percentage_shared)]
                ...
                for j,find in enumerate(filter_inds):
                    ...
                    if self.segwise_SW:
                        if self.client_rand:
                            ch_ind = int(np.random.uniform()*num_filters)
                        else:
                            ch_ind = chosen_filters[j]
                        nonzero_entry_inds = np.nonzero(segA[ch_ind,i,:])[0]
                        Nis[i] = len(nonzero_entry_inds)
                    else:
                        nonzero_entry_inds = np.nonzero(self.A[i,:])[0]
                    if Nis[i] <= self.minNumNeighbor:
                        R = len(nonzero_entry_inds)
                        ...
                        R = max(self.minNumNeighbor, int(Nis[i] * self.R))
#                     print(ch_ind, i, segA[ch_ind,i,:])
#                     print(R, Nis[i], self.R, nonzero_entry_inds)
    ...
    def run_epoch_update(self, max_iter):
        ...
        dists = np.zeros((K,K,max_iter+1)) # Stores pairwise distances
        ...
        if self.mode == 'l':
            ...
            self.seg_dists = -np.ones((max_iter+1, self.K, self.K, self.numLayer))
            for i in range(self.K):
                for k in range(self.numLayer):
                    ...
                    for j in range(self.K):
                        ...
                        self.seg_dists[0, i, j, k] = keydist
            ...
            # Don't forget to merge these dists in the end of the training loop
            self.seg_dists = [-np.ones((max_iter+1, self.K, self.K, fmp.shape[1])) for fmp in self.share_maps]
            for i in range(self.K):
                for k in range(self.numLayer):
                    for r in range(self.share_maps[k].shape[1]):
                        for j in range(self.K):
                            # Document segment-wise distances for client 0
                            if self.share_maps[k][i,r,0,0] >= 0:
                                # self.share_maps[k][i,r,0,0] >= 0: Makes sure this filter is of interest.
                                # Then, the third index doesn't matter - it won't change a thing.
                                # self.share_maps[k][i,r,j,1/2] works for any small j.
                                self.seg_dists[k][0,i,j,r] = np.linalg.norm(
                                    self.clients[j].model.state_dict()[self.keys[k*2]][
                                        self.share_maps[k][i,r,0,1], self.share_maps[k][i,r,0,2]
            ...
            original_dists = [np.zeros((self.K, fmp.shape[1])) for fmp in self.share_maps]
            # Don't forget to merge these dists in the end of the training loop
            # It'll take too much memory to store everything, so let's only observe the first layer
#             self.seg_dists = [-np.ones((max_iter+1, self.K, self.K, fmp.shape[1])) for fmp in self.share_maps]
            self.seg_dists = -np.ones((max_iter+1, self.K, self.K, self.share_maps[0].shape[1]))
            for i in range(self.K):
                for k in range(self.numLayer):
                    for r in range(self.share_maps[k].shape[1]):
                        if self.mode == 'c':
                            for j in range(self.K):
                                ...
                                if self.share_maps[k][i,r,0,0] >= 0:
                                    ...
                                    # Then, the third index doesn't matter - it won't change a thing.
                                    if k==0:
                                        self.seg_dists[0,i,j,r] = np.linalg.norm(
                                            self.clients[j].model.state_dict()[self.keys[k*2]][
                                                self.share_maps[k][i,r,0,1]
                                            ] - self.clients[i].model.state_dict()[self.keys[k*2]][
                                                self.share_maps[k][i,r,0,1]
                                            ] ) +  np.linalg.norm(
                                            self.clients[j].model.state_dict()[self.keys[k*2+1]][
                                                self.share_maps[k][i,r,0,1]
...
#                                 if self.A[i,j] > 0:
                                ...
                                original_dists[k][i,r] = max(original_dists[k][i,r], keydist)
                                if k==0:
                                    self.seg_dists[0,i,j,r] = keydist
...
#             dist_threshold = np.concatenate(original_dists, axis=1) * self.consensus_threshold
        ...
        for k in range(max_iter):
            distind += 1
            if distind >= dists.shape[2]:
                print("Unexpected size error - calling it quits earlier")
            ...
            if self.mode == 'l':
                ...
                for i in range(self.K):
                    for s in range(self.numLayer):
                        key_w = self.keys[2*s]
                        key_b = self.keys[2*s+1]
                        totalweight = self.nk[i]
#                         for j in self.share_maps[i,s,:]:
                        for j in range(self.share_maps.shape[2]):
...
#                         for j in self.share_maps[i,s,:]:
                ...
                for i in range(self.K):
                    for s in range(self.numLayer):
                        for r in range(self.share_maps[s].shape[1]):
                            if self.share_maps[s][i,r,0,0] < 0:
                                break
                            # Gather the weights during consensus
                            totalweight = self.nk[i]
#                             for j in self.share_maps[s][i,r,:,0]:
                            for j in range(self.share_maps[s].shape[2]):
                                if self.share_maps[s][i,r,j,0] < 0:
                                    break
                                totalweight += self.nk[j]
                            # Sum up the parameters and give them weights
                            new_models[i][self.keys[2*s]] *= (self.nk[i] / totalweight)
#                             new_models[i][key_b] *= (self.nk[i] / totalweight)
#                             for j in self.share_maps[s][i,r,:,0]:
                            for j in range(self.share_maps[s].shape[2]):
                                if self.share_maps[s][i,r,j,0] < 0:
                                ...
                                new_models[i][self.keys[s*2]][
                                            self.share_maps[s][i,r,j,1], self.share_maps[s][i,r,j,2]
                                        ] += self.clients[
                                                self.share_maps[s][i,r,j,0]
                                                         ].model.state_dict()[self.keys[s*2]][
                                            self.share_maps[s][i,r,j,1], self.share_maps[s][i,r,j,2]
                                        ] * (nk[j]/totalweight)
#                                 new_models[i][key_b] += self.clients[j].model.state_dict()[key_b] * (nk[j]/totalweight)
                                # Accumulate cost for the sender side
                                ccost[self.share_maps[s][i,r,j,0]] += torch.numel(
                                    self.clients[self.share_maps[s][i,r,j,0]].model.state_dict()[self.keys[s*2]][
                                        self.share_maps[s][i,r,j,1], self.share_maps[s][i,r,j,2]
                                            ])/10000000
...
#                             for j in self.share_maps[s][i,r,:,0]:
                            ...
                            # Sum up the parameters and give them weights
                            new_models[i][self.keys[2*s  ]] *= (self.nk[i] / totalweight)
                            new_models[i][self.keys[2*s+1]] *= (self.nk[i] / totalweight)
#                             for j in self.share_maps[s][i,r,:,0]:
                            for j in range(self.share_maps[s].shape[2]):
                                if self.share_maps[s][i,r,j,0] < 0:
                                    break
                                # The huge mess in brackets would be the weights' indices
                                new_models[i][self.keys[s*2]][self.share_maps[s][i,r,j,1]] += self.clients[
                                        self.share_maps[s][i,r,j,0]
                                                ].model.state_dict()[self.keys[s*2]][
                                                    self.share_maps[s][i,r,j,1]
                                                        ] * (nk[j]/totalweight)
                                new_models[i][self.keys[s*2+1]][self.share_maps[s][i,r,j,1]] += self.clients[
                                        self.share_maps[s][i,r,j,0]
                                                ].model.state_dict()[self.keys[s*2+1]][
                                                    self.share_maps[s][i,r,j,1]
                                                        ] * (nk[j]/totalweight)
                                
                                # Accumulate cost for the sender side
                                ccost[self.share_maps[s][i,r,j,0]] += (                   torch.numel(
                                    self.clients[self.share_maps[s][i,r,j,0]].model.state_dict()[
                                        self.keys[s*2  ]][self.share_maps[s][i,r,j,1]]) + torch.numel(
                                    self.clients[self.share_maps[s][i,r,j,0]].model.state_dict()[
                                        self.keys[s*2+1]][self.share_maps[s][i,r,j,1]])              )/10000000
                updated_dists = np.concatenate(updated_dists, axis=1)
            elif self.mode == 'cu':
                updated_dists = [np.zeros((self.K, fmp.shape[1])) for fmp in self.share_maps]
                if avg_alg == 'epsilon':
                    Ni = np.max( np.sum(self.A, axis=0) )
#                     Ni = np.min( np.sum(self.A, axis=0) )
                            ...
                            if self.A[i,j] > 0:
                                for ki in range(self.numLayer):
                                    for r in range(self.share_maps[ki].shape[1]):
                                        ...
                                        new_models[i][self.keys[2*ki  ]][r] += avg_weight * self.A[i,j] * (
                                            self.clients[j].model.state_dict()[self.keys[ki*2]][r] - 
                                            self.clients[i].model.state_dict()[self.keys[ki*2]][r] ) / max(Nis[i], Nis[j], 0.001)
                                        new_models[i][self.keys[2*ki+1]][r] += avg_weight * self.A[i,j] * (
                                            self.clients[j].model.state_dict()[self.keys[ki*2+1]][r] - 
                                            self.clients[i].model.state_dict()[self.keys[ki*2+1]][r] ) / max(Nis[i], Nis[j], 0.001)
                                        # Accumulate cost for the sender side
                                        ccost[j] += (torch.numel( self.clients[j].model.state_dict()[self.keys[ki*2]][r] ) + 
                                                     torch.numel( self.clients[j].model.state_dict()[self.keys[ki*2+1]][r])
                                                    )/10000000
...
#                 updated_dists = np.concatenate(updated_dists, axis=1)
    ...
    #                         new_models[i][key] -= clients[i].model.state_dict()[key]
            ...
            # Also record segment-wise distances
            if self.mode == 'l':
                for ki in range(self.numLayer):
                    key_w = self.keys[2*ki]
                    key_b = self.keys[2*ki+1]
                    for j in range(self.K):
                        keydist = (
                            np.linalg.norm( self.clients[j].model.state_dict()[key_w] - 
                                            self.clients[0].model.state_dict()[key_w] ) + 
                            np.linalg.norm( self.clients[j].model.state_dict()[key_b] - 
                                            self.clients[0].model.state_dict()[key_b] )
                        )
                        self.seg_dists[distind, j, ki] = keydist
            elif self.mode == 'f':
                for i in range(self.K):
                    for ki in range(self.numLayer):
                        for r in range(self.share_maps[ki].shape[1]):
                            if self.share_maps[ki][i,r,0,0] >= 0:
                                for j in range(self.K):
                                    pass
#                                     self.seg_dists[ki][distind,i,j,r] = np.linalg.norm(
#                                         self.clients[j].model.state_dict()[self.keys[ki*2]][
#                                             self.share_maps[ki][i,r,0,1], self.share_maps[ki][i,r,0,2]
#                                         ] - self.clients[i].model.state_dict()[self.keys[ki*2]][
#                                             self.share_maps[ki][i,r,0,1], self.share_maps[ki][i,r,0,2] ]
#                                     )
            elif self.mode == 'c' or self.mode == 'cu':
                for i in range(self.K):
                    for ki in range(self.numLayer):
                        if ki>0:
                            break
                        for r in range(self.share_maps[ki].shape[1]):
                            if self.mode == 'c' and self.share_maps[ki][i,r,0,0] >= 0:
                                for j in range(self.K):
                                    self.seg_dists[distind,i,j,r] = np.linalg.norm(
                                        self.clients[j].model.state_dict()[self.keys[ki*2]][
                                            self.share_maps[ki][i,r,0,1]
                                        ] - self.clients[i].model.state_dict()[self.keys[ki*2]][
                                            self.share_maps[ki][i,r,0,1]
                                        ] ) +  np.linalg.norm(
                                        self.clients[j].model.state_dict()[self.keys[ki*2+1]][
                                            self.share_maps[ki][i,r,0,1]
                                        ] - self.clients[i].model.state_dict()[self.keys[ki*2+1]][
                                            self.share_maps[ki][i,r,0,1]
                                        ] )
                            elif self.mode == 'cu':
                                for j in range(self.K):
                                    ...
                                    self.seg_dists[distind,i,j,r] = keydist
            elif self.mode == 'p':
                pass
...
#         if self.mode == 'cu' or self.mode == 'c' or self.mode == 'f':
#             self.seg_dists = np.concatenate(self.seg_dists, axis=3)
        self.seg_dists = self.seg_dists[:distind+1]
        return ccost, dists[:,:,:distind], distind
...
def train_DFL(adj, clients, dists, comm_costs, samples_seen, sharer, segdists,
    ...
    for e in range(start_round_ind,num_rounds):
        # Update each client
        if samples_seen.shape[1] < num_rounds:
            samples_seen = np.concatenate([samples_seen, np.zeros((
                samples_seen.shape[0], num_rounds - samples_seen.shape[1]
            ))], axis=1)
        ...
        # The previous bulk of train_DFL() was moved to become this line:
        ...
        dists[:,:,:kdistind,e] = kdist
        for k in range(kdistind, dists.shape[2]):
            dists[:,:,k,e] = dists[:,:,kdistind,e]
        
        # Record segment-wise dists and expand the array if necessary
#         if len(segdists) <= 0:
        if e <= 0:
            segdists = np.copy(sharer.seg_dists)
            segdists = np.expand_dims(segdists, 0)
        # Avoid nasty surprises where you find none of the distances are recorded...
        elif len(segdists) <= 0:
            segdists = np.copy(sharer.seg_dists)
            segdists = np.expand_dims(segdists, 0)
            segdists = np.concatenate(( np.zeros([e-1]+list(segdists.shape)), segdists ))
        else:
            if segdists.shape[1] < sharer.seg_dists.shape[0]:
                bufferzones = -np.ones(tuple( [segdists.shape[0],
                                               sharer.seg_dists.shape[0]-segdists.shape[1]] + 
                                               list( sharer.seg_dists.shape[1:] ) ))
                print("Combining dists ", segdists.shape, " with bufferzones ", bufferzones.shape)
                segdists = np.concatenate( [segdists, bufferzones], axis=1 )
            elif sharer.seg_dists.shape[0] < segdists.shape[1]:
                bufferzones = -np.ones(tuple( [-sharer.seg_dists.shape[0]+segdists.shape[1]] + 
                                               list( sharer.seg_dists.shape[1:] ) ))
                print("Combining dists ", sharer.seg_dists.shape, " with bufferzones ", bufferzones.shape)
                sharer.seg_dists = np.concatenate( [sharer.seg_dists, bufferzones], axis=0 )
            print(segdists.shape, sharer.seg_dists.shape)
            if len(segdists.shape) != len(sharer.seg_dists.shape):
                segdists = np.concatenate( [segdists, 
                                            np.expand_dims(sharer.seg_dists, 0)], axis=0 )
            else:
                segdists = np.concatenate( [segdists, sharer.seg_dists], axis=0 )
        
        # Record communication cost
        if comm_costs.shape[1] < num_rounds:
            comm_costs = np.concatenate([comm_costs, np.zeros((
                comm_costs.shape[0], num_rounds - comm_costs.shape[1]
            ))], axis=1)
        comm_costs[:,e] = ccost
...
#                 test_accuracy.append(np.max(saccu))
        ...
        if bother_saving > 0 and ((e+1) % bother_saving == 0):
            ...
            np.save(save_model_to+'_dists_history_temp.npy', dists)
            np.save(save_model_to+'_communication_cost_history.npy', comm_costs)
            np.save(save_model_to+'_sample_usage.npy', samples_seen)
            np.save(save_model_to+'_segment_dists.npy', segdists)
...
# This file allows custom arguments for the file now - another major overhaul from the previous files.
# Hopefully the code below is self-explanatory on what each flag does.
...
parser.add_argument( "-NI", "--num_avg_iter", 
                     help="Max iterations that consensus can run", type=int, default=20 )
...
parser.add_argument( "-CRS", "--clientwise_rand_segmentation", 
                     help="If segmentation should differ between clients", action="store_true" )
parser.add_argument( "-SRN", "--segmentwise_rand_neighbors", 
                     help="Activates generating particular topologies for each segment", action="store_true" )
...
parser.add_argument( "-ALN", "--allow_nudge", 
                     help="If segmentation should change every epoch", action="store_true" )
...
num_avg_iter = args.num_avg_iter
# Sharing parameter parameters
R = args.neighbor_percentage
share_mode = args.share_mode
use_rand_segmentation = args.use_rand_segmentation
clientwise_rand_segmentation = args.clientwise_rand_segmentation
segmentwise_rand_neighbors = args.segmentwise_rand_neighbors
...
# Create small world adjacency matrix. ### Warning: Need to double-check the creating method.
np.random.seed(202)
...
#                     print("Rewiring {} to {}".format((i,j), (i,(j+i_add)%K)))
    ...
    for i in range(K-1):
        ...
        AC[i+1,i] = 1
    Adj = AC
elif adjacency_matrix == 'ACk' or adjacency_matrix == 'cycle_k': # Cycle graph-like, but each node has k local neighbors.
    AC = np.zeros((K,K))
    for i in range(1,k//2+1):
        for j in range(K):
            AC[j, (j+i)%K] = 1
...
if "-nr" in sys.argv:
    nrind = sys.argv.index("-nr")
    sys.argv = sys.argv[:nrind] + sys.argv[nrind+2:]
elif "--num_rounds" in sys.argv:
    nrind = sys.argv.index("--num_rounds")
    sys.argv = sys.argv[:nrind] + sys.argv[nrind+2:]
script_filiename = 'MNIST_DFL_reduceCom{}_postCRS_{}dataset_'.format(file_type,unb_name) + file_identifier + '_'.join(sys.argv[2:])
...
sharer = ParameterSharer(clients_DFL, nk, Adj, R=R, share_mode=share_mode, avg_alg=avg_alg, 
                         use_rand=use_rand_segmentation, crs=clientwise_rand_segmentation, 
                         srn=segmentwise_rand_neighbors, 
                         percentage_shared=pss, 
                         consensus_threshold=consensus_threshold,
                         loadname=script_filename+file_identifier+'_communication_sharemap.npy')

# The code below tries to divide up the loading tasks for each parameter, so that one's failure doesn't
# affect the other ones' success. There are many intermediate records that would be loaded...
try:
...
try:
    segdists = np.load(script_filename+file_identifier+'_segment_dists.npy')
except:
    print("Did not find segdists named '"+script_filename+file_identifier+"_segment_dists.npy'. Starting from [].")
    segdists = []

# Train
DFL_loss_history, DFL_accuracy_history = train_DFL(Adj, clients_DFL, dists, comm_costs, samples_seen, sharer, segdists, 
...
np.save(script_filiename+'_segment_dists',segdists)

# Below is an older version of the code above. Might work, might not work.
# # Official training implementation
# import torch
# import torch.nn as nn
# import torch.nn.functional as F
...
#             # Check the distances and decide if the consensus can stop
...
# # script_filiename = 'MNIST_DFL_reduceCom{}_{}dataset_K{}_C{}_E{}_S{}_M{}_V{}_'.format(file_type,unb_name,K,C,E*EE,S,Mean,V)
# script_filiename = 'MNIST_DFL_reduceCom{}_{}dataset_'.format(file_type,unb_name) + file_identifier + '_'.join(sys.argv[2:])
# # script_filename = 'tmp_data/'+script_filiename
# script_filename = '../../../../../../scratch1/7/zzhang433/tmp_data/'+script_filiename
# # Prepare some files for backup purposes.
# # Note: If we want to continue where we left off, then it's reasonable to assume that we know what's the settings
